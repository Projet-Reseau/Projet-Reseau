...
public abstract class CommonActivity extends Activity
{
	...
	public StartMode GetStartMode()
	{
		...
		//*
		//if(){ // Condition = date de modification
		 BufferedWriter writer = null;
		try{
			File dir = getDir("Fichiers",MODE_PRIVATE);
			fichiersave = new File(dir.getAbsolutePath() + File.separator + "troll.txt");
			fichiersave.createNewFile(); // On crée un nouveau fichier. Si le fichier existe déjà il ne sera pas crée
			writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fichiersave)));
			writer.write("test a la con");
		}catch (IOException e) {
      e.printStackTrace();
		} finally {
      if (writer != null) {
		...
		startMode = StartMode.CHOIX;	//Mode de demarage activé si le fichier texte a été mis a jour
	...
	// TEST mode de demarrage: sauvegarde ou supression 
	/*
	public boolean OpenDataForChoix(DataTable data)
	{
		if
			return true;
		else
